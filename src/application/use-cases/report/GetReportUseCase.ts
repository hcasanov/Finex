...
import type { GetReportInput } from "@/application/validators/schemas/reportSchemas";
...
export class GetReportUseCase {
  ...
  async execute(input: GetReportInput): Promise<ReportDetailsDTO> {
    const report = await this.reportRepository.findById(input.id);

    if (!report) {
      throw new ReportNotFoundError(input.id);
    }
